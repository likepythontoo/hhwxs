...
const RecruitmentManagement = () => {
  ...
  const exportCSV = () => {
    ...
    const rows = items.map(r => [
      ...
      r.preferred_department || "", r.status === "approved" ? "通过" : r.status === "rejected" ? "拒绝" : "待审",
    ...
    a.href = url; a.download = `招新申请_${new Date().toLocaleDateString("zh-CN")}.csv`;
  ...
  const filtered = items.filter(i => !search || i.name.includes(search) || i.student_id?.includes(search) || i.college?.includes(search));
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {item.preferred_department && <span className="rounded-full bg-accent/20 px-2 py-0.5 text-[10px] text-accent-foreground">{item.preferred_department}</span>}
                  </div>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {item.student_id && `学号 ${item.student_id} · `}{item.college && `${item.college} · `}{item.phone}
                  </p>
                  {item.self_intro && <p className="mt-1.5 line-clamp-1 text-xs text-muted-foreground/80">{item.self_intro}</p>}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 p-4 backdrop-blur-sm" onClick={() => setViewing(null)}>
          <div className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-border bg-card p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
              ...
              <p className="col-span-2"><span className="text-muted-foreground">意向部门：</span>{viewing.preferred_department || "-"}</p>
            </div>
            {viewing.self_intro && (
              <div className="mt-4">
                <p className="mb-1 text-xs font-medium text-muted-foreground">自我介绍</p>
                <div className="rounded-lg bg-secondary/50 p-3 text-sm">{viewing.self_intro}</div>
              </div>
            )}
            {viewing.literary_works && (
              <div className="mt-3">
                <p className="mb-1 text-xs font-medium text-muted-foreground">文学作品/经历</p>
                <div className="whitespace-pre-wrap rounded-lg bg-secondary/50 p-3 text-sm">{viewing.literary_works}</div>
              </div>
            )}
            <div className="mt-4">
              <label className="mb-1.5 block text-xs font-medium text-muted-foreground">审核备注</label>
              <textarea value={reviewNotes} onChange={(e) => setReviewNotes(e.target.value)} className="w-full rounded-lg border border-border bg-secondary/30 px-3 py-2 text-sm focus:border-primary focus:outline-none" rows={2} placeholder="审核意见..." />
            </div>
